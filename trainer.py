...
class Executor:
    ...
    def execute(self, model_generator):
        ...
        model_arch.fit(epochs = self.num_epochs, lr = self.lr, model_generator = model_generator, train_loader = dl_train, val_loader = dl_val,
            opt_func = self.opt_func,  criterion = self.criterion, batch_size = int(self.batch_size),  accumulation_steps = self.accumulation_steps, 
            chunk_size = self.chunk_size, patience = self.patience, minio_client=self.minio_client, minio_bucket=self.minio_client.bucket
        )
